...
public class Trick implements Serializable
{
	...
	{
		// Player can only play a card on their turn
		if (getNextToPlay() != p.getDirection())
		{
			throw new IllegalMove("It is " + getNextToPlay().toString() + "'s turn to play. \nPlease Wait for yout turn.");
		}
		// Player can only play a card that he is present in their hand
		if (!p.getHand().contains(c))
		{
			throw new IllegalMove("You do not have " + c.toString() + " in your hand.");
		}
		if (p.getDirection() != lead) // If player not in lead
		{
			Card.Suit leadSuit = getCardPlayed(lead).suit();
			Set<Card> match = p.getHand().matches(leadSuit); // Card that follows the lead
			// Player must to play a card follows the suit if they have one
			if (!match.isEmpty() && !match.contains(c))
			{
				throw new IllegalMove(
						"Please follow the lead suit.");
			}
		}
