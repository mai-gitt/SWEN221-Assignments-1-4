...
public abstract class AbstractCardGame implements CardGame, Serializable
{
	...
	/**
	 * Keeps track of the number of tricks each player has won in the current round.
	 */
	protected final Map<Player.Direction, Integer> tricks = new HashMap<Player.Direction, Integer>();

	/**
	 * Keeps track of the player scores. In some games, this may equal the number of
	 * tricks. In others, this may include certain bonuses that were obtained.
	 */
	...
	public CardGame clone()
	{
		...
		// deep clone.
		ByteArrayOutputStream buffer = new ByteArrayOutputStream();	// Making a memory buffer
		ObjectOutputStream objOutput = null;
		ObjectInputStream objInput = null;
		CardGame clone = null;
		try
		{
			objOutput = new ObjectOutputStream(buffer);
			objOutput.writeObject(this); // Serialisation
			
			objInput = new ObjectInputStream(new ByteArrayInputStream(buffer.toByteArray()));
			clone = (CardGame) objInput.readObject(); // De-serialisation
		} catch (Exception e)
		{
